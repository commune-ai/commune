...
def module_stats():
    module_name = request.args.get('module_name')
    module = c.module()  # Assuming c.module() is defined elsewhere and works correctly.
    ...
    try:
        ...
        # Check if the result is a DataFrame and convert
        if isinstance(modules_tree, pd.DataFrame):
            # Convert DataFrame to a dictionary in a JSON-friendly format
            modules_tree = modules_tree.to_dict(orient='records')  # 'records' format means list of dicts
...
def infos():
    ...
    infos = {}
    if not module_name:
        infos['total_module'] = len(c.modules())
        infos['total_user'] = random.randrange(start=1,stop=1000)
        infos['module_pending'] = random.randrange(start=1,stop=100)
        infos['review_request'] = random.randrange(start=1,stop=100)

        return jsonify(infos)
    else :
        return jsonify({"error":""}), 500

@app.route('/modules/history', methods=['GET'])
def history():
    module_name = request.args.get('module_name')
    history_array = np.random.randint(60,101, size=7)


    if not module_name:
        history = {'history': history_array.tolist()}

        return jsonify(history)
    else :
        return jsonify({"error":""}), 500

@app.route('/servers/analytics', methods=['GET'])
def server_analytics():
    module_name = request.args.get('module_name')
    history_array = np.random.randint(60,101, size=7)
    servers = c.servers()
    if not module_name:
        server_anal = {'history': history_array.tolist()}

        return jsonify(server_anal)
    else :
        return jsonify({"error":""}), 500
