...
fn test_registration_difficulty_adjustment() {
	new_test_ext().execute_with(|| { 
        ...
        assert_eq!( SubtensorModule::get_hotkey_for_net_and_uid( netuid, 0 ).unwrap(), hotkey0 );
		assert_eq!( SubtensorModule::get_hotkey_for_net_and_uid( netuid, 1 ).unwrap(), hotkey1 );
		assert_eq!( SubtensorModule::get_hotkey_for_net_and_uid( netuid, 2 ).unwrap(), hotkey2 );

		assert_eq!( SubtensorModule::get_subnet_n(netuid), 3); // All 3 are registered.
        ...
        assert_eq!( SubtensorModule::get_hotkey_for_net_and_uid( netuid, 0 ).unwrap(), hotkey0 + 1); // replace 0
		assert_eq!( SubtensorModule::get_hotkey_for_net_and_uid( netuid, 1 ).unwrap(), hotkey1 + 1); // replace 1
		assert_eq!( SubtensorModule::get_hotkey_for_net_and_uid( netuid, 2 ).unwrap(), hotkey2 + 1); // replace 2
