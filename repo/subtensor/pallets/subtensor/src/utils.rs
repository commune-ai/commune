...
impl<T: Config> Pallet<T> {
    ...
    pub fn set_pruning_score_for_uid( netuid:u16, uid: u16, pruning_score: u16 ) {
        ...
        log::info!("SubnetN::<T>::get( netuid ) = {:?}", SubnetN::<T>::get( netuid ) );
        log::info!("uid = {:?}", uid );
        assert!( uid < SubnetN::<T>::get( netuid ) );
    ...
    pub fn do_sudo_set_max_allowed_uids( origin:T::RuntimeOrigin, netuid: u16, max_allowed_uids: u16 ) -> DispatchResult {
        ...
        ensure!( Self::get_subnet_n(netuid) < max_allowed_uids, Error::<T>::MaxAllowedUIdsNotAllowed);
