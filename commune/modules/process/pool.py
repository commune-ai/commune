...
- creates a uniquely numbered WorkItem and adds it to the "Work Items" dict
- adds the id of the WorkItem to the "Work Ids" queue
...
- reads ResultItems from "Result Q", updates the future stored in the
  "Work Items" dict and deletes the dict entry

Process #1..n:
- reads _CallItems from "Call Q", executes the calls, and puts the resulting
  ResultItems in "Result Q"
"""
...
def _sendback_result(result_queue, work_id, result=None, exception=None,
    ...
    try:
        result_queue.put(ResultItem(work_id, result=result,
                                     exception=exception, exit_pid=exit_pid))
    except BaseException as e:
        exc = _ExceptionWithTraceback(e, e.__traceback__)
        result_queue.put(ResultItem(work_id, exception=exc,
                                     exit_pid=exit_pid))
...
def _process_worker(call_queue, result_queue, initializer, initargs, max_tasks=None):
    ...
    Args:
        ...
        result_queue: A ctx.Queue of ResultItems that will written
            to by the worker.
...
class _ExecutorManagerThread(threading.Thread):
    ...
    def __init__(self, executor):
        ...
        # A ctx.SimpleQueue of ResultItems generated by the process workers.
    ...
    def process_result_item(self, result_item):
        ...
        # worker that exited gracefully or a ResultItem

        # Received a ResultItem so mark the future as completed.
...
class ProcessPoolExecutor(_base.Executor,c.Module):
    ...
    @property
    def num_tasks(self):
        return self._call_queue.__sizeof__()
