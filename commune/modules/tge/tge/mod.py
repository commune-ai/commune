...
class tge:
    ...
    def plots(self, *args, **kwargs):
        ...
        # Lock Multiplier Curve (0x at 0 days, 1x at 1 year, 4x at 2 years)
        lock_days = np.linspace(0, 730, 100)  # 0 to 2 years in days
        multipliers = np.where(lock_days == 0, 0, (lock_days / 365) ** 2)  # Quadratic curve: (years)^2
        
        fig_lock_multiplier = go.Figure()
        fig_lock_multiplier.add_trace(go.Scatter(
            x=lock_days / 365,  # Convert to years for x-axis
            y=multipliers,
            mode='lines',
        ...
        # Vesting Schedule Visualization (Linear vesting over lock period)
        vesting_periods = [365, 730]  # 1 year and 2 years in days
        fig_vesting = go.Figure()
        
        for period_days in vesting_periods:
            days = np.linspace(0, period_days, 100)
            multiplier = (period_days / 365) ** 2
            base_tokens = 1000000  # Example: 1M base tokens
            vested_tokens = (days / period_days) * base_tokens * multiplier
