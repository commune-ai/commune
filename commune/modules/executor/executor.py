...
class TaskExecutor(c.Module):
    ...
    def submit(self, fn: Callable, args:list=None, kwargs:dict=None) -> Future:
        args = args or []
        kwargs = kwargs or {}
    ...
    def adjust_thread_count(self):
        ...
        if num_threads < self.max_workers:
            ...
            args = (weakref.ref(self, weakref_cb),self.work_queue)
            t = threading.Thread( name=thread_name, target=self.worker,args=args)
            t.daemon = True
            t.start()
