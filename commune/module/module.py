...
class c:
    ...
    @classmethod
    def cleaar_commit_history(cls, cwd=None):
        # 1. Navigate to the root of your repository (assuming c.cmd can change directories)
        cwd = c.libpath if cwd == None else cwd
        c.cmd('cd {cwd}')  # Replace with your repository path
        
        # 2. Checkout to a temporary branch
        c.cmd('git checkout --orphan temp_branch', cwd=cwd)
        
        # 3. Add all the files & commit them
        c.cmd('git add -A', cwd=cwd)
        c.cmd('git commit -am "Initial commit"', cwd=cwd)
        
        # # 4. Delete the old branch (assuming it's 'main', if not replace 'main' with your branch name)
        # c.cmd('git branch -D main', cwd=cwd)
        
        # # 5. Rename the temporary branch to 'main'
        # c.cmd('git branch -m main', cwd=cwd)
        
        # 6. Force push to the remote repository (assuming remote is named 'origin')
        c.cmd('git push -f origin main', cwd=cwd)
