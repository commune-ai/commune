...
class MetricMap(Metric):
        ...
        if metrics == None:
            ...
            for metric_key, metric in metrics.items():
                if isinstance(metric, dict):
                    ...
                    metric_class = self.default_metric_map[metric_path]
                    metric = metric_class(**metric)
                else:
                    metric_class = self.default_metric_map[self.default_metric_path]
    ...
    def update(self, key:str, value: Any = None, 
        ...
        if key not in self.metrics:
            metric_class = self.default_metric_map[metric]
    ...
    def rm_metric(self, name:str) -> str:
        ...
        return name
    def add_metric(self, metric:, params: dict = None, refresh: bool = False) -> None:
        if refresh:
            self.metrics.pop(key, None)
        metric_class = cls.get_default_metric_map()[metric_path]
        params = params if params is not None else {}
        metric_class(**params)
