...
crypto_types = ['ecdsa', 'solana', 'sr25519', 'ed25519']

@pytest.mark.parametrize('crypto_type', crypto_types)
def test_encryption(crypto_type, values = [10, 'fam', 'hello world']):
    cls = c.module('key')
    for value in values:
        value = str(value)
...
@pytest.mark.parametrize('crypto_type', crypto_types)
def test_encryption_with_password(crypto_type, value = 10, password = 'fam'):
    cls = c.module('key')
    value = str(value)
    key = cls.new_key(crypto_type=crypto_type)
...
@pytest.mark.parametrize('crypto_type', crypto_types)
def test_key_encryption(crypto_type, test_key='test.key'):
    self = c.module('key')
...
@pytest.mark.parametrize('crypto_type', crypto_types)
def test_key_management(crypto_type, key1='test.key' , key2='test2.key'):
    key = c.module('key')
    if key.key_exists(key1):
        key.rm_key(key1)
    if key.key_exists(key2):
        key.rm_key(key2)
    key.add_key(key1, crypto_type=crypto_type)
    k1 = key.get_key(key1, crypto_type=crypto_type)
    assert key.key_exists(key1), f'Key management failed, key still exists'
    key.mv_key(key1, key2, crypto_type=crypto_type)
    k2 = key.get_key(key2, crypto_type=crypto_type)
    ...
    key.mv_key(key2, key1, crypto_type=crypto_type)
...
@pytest.mark.parametrize('crypto_type', crypto_types)
def test_key_encryption(crypto_type, password='1234'):
    cls = c.module('key')
    path = 'test.enc'
