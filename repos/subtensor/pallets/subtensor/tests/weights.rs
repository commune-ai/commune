...
fn test_weights_err_has_duplicate_ids() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(SubtensorModule::get_subnet_n(netuid), 4);
...
fn test_weights_err_max_weight_limit() { //TO DO SAM: uncomment when we implement run_to_block fn
	new_test_ext().execute_with(|| { 
		...
		assert_eq!(SubtensorModule::get_subnet_n(netuid), 2);
		step_block(1);
		...
		assert_eq!(SubtensorModule::get_subnet_n(netuid), 5);
		step_block(1);
...
fn test_check_len_uids_within_allowed_within_network_pool() {
	new_test_ext().execute_with(|| {
		...
		let max_allowed: u16 = SubtensorModule::get_subnet_n(netuid);
