...
impl<T: Config> Pallet<T> {
    ...
    pub fn do_sudo_registration( 
        ...
        let current_subnetwork_n: u16 = Self::get_subnet_n( netuid );
    ...
    pub fn passes_network_connection_requirement( netuid_a: u16, hotkey: &T::AccountId ) -> bool {
        ...
        for (netuid_b, exists) in NetworksAdded::<T>::iter() {
            ...
            if exists && Self::network_connection_requirement_exists( netuid_a, netuid_b ){
                ...
                let subnet_n: u16 = Self::get_subnet_n( netuid_b );
                ...
                let topk_percentile_value: I32F32 = I32F32::from_num( n_better_prunning_scores ) / I32F32::from_num( Self::get_subnet_n( netuid_b ) );
    ...
    pub fn get_neuron_to_prune(netuid: u16) -> u16 {
        ...
        if Self::get_subnet_n( netuid ) == 0 { return 0 } // If there are no neurons in this network.
        for neuron_uid_i in 0..Self::get_subnet_n( netuid ) {
