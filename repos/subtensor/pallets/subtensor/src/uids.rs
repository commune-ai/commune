...
impl<T: Config> Pallet<T> { 
    ...
    pub fn replace_neuron( netuid: u16, uid_to_replace: u16, new_hotkey: &T::AccountId, block_number:u64 ) {

        log::debug!("replace_neuron( netuid: {:?} | uid_to_replace: {:?} | new_hotkey: {:?} ) ", netuid, uid_to_replace, new_hotkey );
    ...
    pub fn append_neuron( netuid: u16, new_hotkey: &T::AccountId, block_number:u64 ) {
        ...
        let next_uid: u16 = Self::get_subnet_n( netuid );
        log::debug!("append_neuron( netuid: {:?} | next_uid: {:?} | new_hotkey: {:?} ) ", netuid, new_hotkey, next_uid );

        // 2. Get and increase the uid count.
        SubnetN::<T>::insert( netuid, next_uid + 1 );
